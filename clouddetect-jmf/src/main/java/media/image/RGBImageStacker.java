 ...
 * @author Mike
 * 
 * TODO remove text progress bar (add listeners?) TODO uses JMF RGBFormat, try
 * to convert this to standard java if possible
 */
public class RGBImageStacker {
    ...
    public short[] getOriginalStackData() {
        ...
        for (int counter = 0; counter != stackedData.length; counter++) {
            tmp = (short) Median.find(stackedData[counter], 0,
                    FRAMES_TO_STACK - 1);
    ...
    /**
     * Data is median stacked, converted to B/W and scaled so that the smallest
     * possible change is 1 unit TODO recalculate scaling and document
     * 
     * @return
     */
