...
/**
 * This class offers the kMean binning algorithm. Some important
 * observations about this algorihtm can be made: <ul>
 * <li>the elements need not be sorted, but sorting them considerable
 * reduces the # iterations needed to complete the process;</li>
 * <li>the result of the algorithm <em>depends</em> very much on the
 * ordering of the elements, different orderings will result in
 * different bins!</li></ul>
 *
 * @author <a href="mailto:Peter.DeMaeyer@synes.com">Peter De Maeyer</a> (original)
...
public class KMeans {
  ...
  /**
   * Creates a new <code>KMeans</code> instance. The elements need not
   * be sorted, but the resulting bin data will be!
   *
...
//             this.mBinMeans);
   ...
   * @param aBinMinima a <code>double[]</code> value
   * @param aBinMaxima a <code>double[]</code> value
   * @param aBinMeans a <code>double[]</code> value
   */
  public static final void sortBins(int aBinCount,
                                    int[][] aBins,
                                    double[] aBinMinima,
                                    double[] aBinMaxima,
                                    double[] aBinMeans) {
    ...
    for (int i = 0; i != aBinCount; i++) {
      bins[i] = new Bin(aBins[i], aBinMinima[i], aBinMaxima[i], aBinMeans[i]);
    }
  ...
  public static final void sortBins(int aBinCount,
                                    int[][] aBins,
                                    double[] aBinMeans) {
    sortBins(aBinCount, aBins, new double[aBinCount], new double[aBinCount], aBinMeans);
  }

  /**
   * Performs the kMeans binning algorithm on an array of elements.
   *
   * @param aElements an array of elements of arbitrary length (must
   * not be <code>null</code>)
   * @param aBinCount a # bins (must be <code>&gt;= 0</code>)
   * @param aBinMeans an array of bin means, where the resulting bin
   * means will be copied in (out parameter). The length of this array
   * must be equal to the # bins
   * @param aBins an array of <code>int[]</code>, where the indexes in
   * the original elements array will be copied in. The length of the
  ...
  /**
   * Performs the kMeans binning algorithm on an array of elements,
   * without calculating bin minima/maxima.
   *
   * @param aElements a <code>double[]</code> value
   * @param aBinCount an <code>int</code> value
   * @param aBins an <code>int[][]</code> value
    ...
    do {
      ...
      for (i = 0; i != aBinCount; i++) {
        ...
        for (j = 0; j != binSize;) {
          ...
          if (index != i) {
            bin.removeElementAt(j); // Remove the element from the bin...
            binSize--; // Don't forget to update the bin size...
            aBins[index].add(jndex); // Put the element in the new bin...
            cont = true;

            means(aElements, aBinCount, aBins, aBinMeans); // Recalculate the means...
          } else {
            j++;
          }
        }
  ...
  private static class Bin {
    ...
    public Bin(int[] aBin, double aBinMinimum, double aBinMaximum, double aBinMean) {
      this.mBin = aBin;
      this.mBinMinimum = aBinMinimum;
      this.mBinMaximum = aBinMaximum;
      this.mBinMean = aBinMean;
      // DEBUG
//        System.out.println("bin = " + com.synes.util.ArrayUtils.toString(this.mBin));
