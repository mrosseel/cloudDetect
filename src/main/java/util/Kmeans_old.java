...
/**
*This class offers the functionality for the k-means non-hierarchical clustering algorithm.  Non-hierarchical
*clustering algorithms are characterised by an initial segregation of the objects in K clusters, which are 
*improved iteratively.
*
*The k-means algorithm works as follows :
*	1) divide the objects in K groups and calculate the mean value of each cluster.
*	2) for every object : calculate the distance to the mean value of every cluster and assign the object
*		to the cluster with the nearest mean.  If an object is assigned to another cluster, the means of the 2 should be
*		recalculated.  
*	3) repeat step 2 until no reallocation is necessary.
*
*In order to improve the performance of the algorithm, the initialization phase (1) sorts the row of objects 
*that will be clustered, before dividing them into K clusters, each containing the same number of objects.
*
*Standard this implementation supports only a 1-dimensional clustering problem, since
*in the current implementation the distance between 2 objects (ints) a and b is defined as (|a-b|),
*but it can easily be extended by implementing a different distance function (e.g. a euclidean of Mahalanobis distance function).
*
*
...
{	
  ...
  class Cluster
  {
     ...
     *
     * @param   aData  data to be clustered in k groups
     * @param   aK  indicating in how many clusters data need to be divided
     */
    public Kmeans_old(short[] aData, int aK)
    {
        ...
        if(aData.length == 0)
            throw new IllegalArgumentException("data must contain at least one record");
	...
	/**
	 * Returns the borders between the clusters. (In reality these are the maxima of each cluster,
	 * with exception of the last one).
	 *
	 * @return  array of ints, each of them representing a border between two consecutive clusters
	 */
	...
	/**
	 * Returns the sizes of the clusters. How much elements are there in each cluster.
	 *
  ...
  public int getClusterCount()
  {
	 ...
	 *
	 * @param   aClusterIndex  index of a cluster
	 ...
	 *
	 * @param   aValue  value to be classified in a cluster
	...
	public void init()
	{
	  ...
	  if ((distinctData.size() == 0) || (currentValue != distinctData.get(distinctData.size()-1)))
	  {
	  ...
	  if (distinctData.size() <= this.mK)
	  {
	    ...
	    for (i = 0; i != distinctData.size(); i++)
	      this.mClusters[i] = new Cluster(distinctData.get(i), distinctDataCounts.get(i));
	  }
	  else
	  {
	    int value;
	    ...
	    for (i = 0; i != distinctData.size(); i++)
	    {
	      ...
	      if (((this.mClusters[clusterIndex].getSize()+count) <= clusterMaxSize) || (clusterIndex == this.mK-1))
	 ...
	 *
	 * @param   aOldClusterIndex  index of the original cluster
	 * @param   aNewClusterIndex  index of the new cluster
	 * @param   aPosition  position of the value in the original cluster
	 */
	private void switchCluster(int aOldClusterIndex, int aNewClusterIndex, int aPosition)
	{
  ...
  /**
   * Helper method : it calculates the distance between two ints a and b as (|a-b|), typically used to determine the distance
   * between an object and the mean of a cluster.
   * In it present implementation only 1-dimensional samples are supported.
   *
   * For multi-dimensional situations other algorithms should be used, like the euclidean or mahalanobis distance.
   * The input parameters should the be represented by int[].
   *
   */
	 ...
	 *
	 * @param   aArray  sorted array of ints
	 * @param   aValue  value to be looked for its position in the array
