...
public class IntVector implements Serializable {
     ...
     * capacity increment is smaller than or equal to 0, the capacity of the
     * vector is intd each time it needs to grow.
     */
     ...
     *                thrown when the specified initial capacity is negative.
     */
     ...
     * 
     * @param array
     *            contains the elements from which the vector is constructed.
     */
     ...
     * at index k in this vector is copied into component k of
     * <code>anArray</code>. The array must be big enough to hold all the
     * objects in this vector, else an <code>IndexOutOfBoundsException</code>
     * is thrown.
     * 
     * @param anArray
     *            the array into which the components get copied.
     * @exception ArrayIndexOutOfBoundsException
     *                thrown when the array is too small to hold all the
     ...
     * array, kept in the field <code>elementData</code>, with a smaller one.
    ...
    public void ensureCapacity(int minCapacity) {
        if (this.elementData.length < minCapacity) {
            int newCapacity = (this.capacityIncrement > 0) ? this.capacityIncrement
     ...
     * is less than the current size, all components at index
     ...
     *         in the field <code>elementData</code> of this vector.
     */
    public int capacity() {
    ...
    /*
     * public Enumeration elements() { Enumeration enum = new Enumeration();
     *  // Needs work
     * 
     * return enum; }
     */
     ...
     *         same as a component in this vector, as determined by
     ...
     * optimizations. First of all, an extra procedure call is avoided, and
     * second of all, comparison can be done safely with != instead of <, which
     * is slightly but noticeably faster.
     * 
     * @param elem
     *            a int.
    ...
    public int indexOf(int elem, int index) {
        for (int i = index; i < this.elementCount; i++)
            // < instead of != to avoid infinite loop when index argument >
            // this.elementCount
            if (this.elementData[i] == elem)
                return i;
     ...
     * This method is identical in functionality to the <code>get</code>
     * method, but slightly faster since the latter relies upon the former and
     * thus requires an extra function call.
     * 
     * @param index
     *            an index into this vector.
     * @return the component at the specified index.
     * @exception ArrayIndexOutOfBoundsException
     *                if the index is negative or not less than the current size
     *                of this Vector object. given.
     */
     ...
     * to be the specified object. The previous component at that position is
     * discarded. The index must be a value >= 0 and < the current size of the
     * vector.<br>
     * This method is identical in functionality to the <code>set</code>
     * method.
     * 
     * @param obj
    ...
    /**
     * Deletes the component at the specified <code>index</code>. Each
     * component in this vector with an index greater or equal to the specified
     * index is shifted downward to have an index one smaller than the value it
     * had previously. The size of this vector is decreased by 1.<br>
     * The <code>index</code> must be a value >= 0 and < the current size of
     * the vector.<br>
    ...
    public void removeElementAt(int index)
        ...
        if (copyLength != 0) // Dunno what would happen if arraycopy(., ., .,
                                // ., 0) is done, just avoiding trouble here
            System.arraycopy(this.elementData, index + 1, this.elementData,
                    index, copyLength); // Shifting right part to left
        this.elementCount--;
    }
     ...
     * index greater or equal to the specified index is shifted upward to have
     * an index one greater than the value it had previously.<br>
     ...
     * <code>add(Object, int)</code> method. Note that the add method reverses
    ...
    public void insertElementAt(int obj, int index)
        ...
        if (copyLength != 0) // Dunno what would happen if arraycopy(., ., .,
            ...
            System.arraycopy(this.elementData, index, this.elementData,
                    index + 1, copyLength); // Shifting right part to right
        this.elementData[index] = obj; // Inserting the element at the
                                        // designated position
        this.elementCount++;
    }
    ...
    public void addElement(int obj) {
        if (this.elementCount == this.elementData.length) // If internal array
                                                            // is full, expand
                                                            // the array
            expand();
     ...
     * downward to have an index one smaller than the value it had previously.<br>
     ...
     * This method is identical in functionality to the <code>clear</code>
     * method. <br>
     * Note that this operation does <i>not</i> reduce the capacity, the
     * capacity remains!
     */
     ...
     * internal data array of this Vector object.<br>
     * Note that a clone does <i>not</i> copy the capacity! The initial
     * capacity of a clone is always the same as its size. (This is also the
     * case with regular Java <code>Vector</code>s).
     * 
    ...
    public Object clone() {
        ...
        System
                .arraycopy(this.elementData, 0, elementData, 0,
                        this.elementCount);
     ...
     * The returned array is <i>not</i> a reference to the internal array, but
     * a freshly created array!
     * 
     ...
     * 
     * @param intVector
     *            the vector with which to compare this one.
     * @return <code>true</code> if both vectors have the same content;
     ...
     * @param toIndex
     *            exclusive.
     * @return a new <code>IntVector</code> with the elements of the specified
     *         range.
     */
    public IntVector subVector(int fromIndex, int toIndex)
        ...
        if (copyLength != 0)
            System.arraycopy(this.elementData, fromIndex, subList, 0,
                    copyLength);
     ...
     * Such an iterator works in the same way as a normal iterator, except that
     * it returns <i>typed</i> elements (ints) instead of objects. For this
    ...
    private void expand() {
        int newCapacity = this.elementData.length
                + ((this.capacityIncrement > 0) ? this.capacityIncrement
                        : ((this.elementData.length != 0) ? this.elementData.length
                                : 1));
        int[] newElementData = new int[newCapacity];
    ...
    private class IntIterator implements util.IntIterator {
         ...
         * 
         * @param v
         *            the <code>IntVector</code> from which the iterator has
         *            to be created.
         */
        public IntIterator(IntVector v) {
            this.v = v;
            this.iterator = -1; // next() must be called to get the first
                                // element!
            this.removedFlag = true; // Unless next is called at least once,
         ...
         * (this means that strange things can happen).
         * 
        ...
        public void remove() throws IllegalStateException {
            if (this.removedFlag)
                throw new IllegalStateException(
                        "This element has already been removed, next() was never called, or simply no elements left!");

            v.removeElementAt(this.iterator);
            this.iterator--; // After removal, all other elements have
                                // shifted, so adjust iterator accordingly!
